#!/bin/bash
# mbc20_daemon.sh - Uniwersalny menedzer headless daemona MBC20 / Moltbook
...
if isinstance(d,dict) and 'profiles' in d:
  ...
  print('  - profiles.json: skonwertowano dict.profiles -> lista')
...
if n_desc:
    ...
    text += '\n\n' + replacement_desc
    print("    Dodano get_post_description() uzywajacy extra_comment.")
...
MOLTBOOK_API_NAME=
EOF
    echo "  - utworzono nowy .env: $ENV_FILE"
  else
    echo "  - wykryto istniejaca .env: $ENV_FILE"
  fi
  ...
  if [ -z "$OLD_MOLT" ]; then
    echo "MOLTBOOK_API_KEY w .env jest pusty – musisz podac wartosc."
    while true; do
...
EOF
  ...
  if [ "$USE_SYSTEMD" -eq 1 ]; then
    echo "  - wykryto systemd, tworzenie uslugi..."
...
SVCEOF
    ...
    echo "  - wykryto SysVinit, tworzenie /etc/init.d/${SERVICE_NAME}..."
...
add_profile() {
  ...
  read -p "Ilosc per mint (np. 100): " AMOUNT
...
edit_env() {
  ...
  echo "Aktualne klucze (maskowane):"
...
reindex_history() {
  ...
  if [ -f "${AUTO_DIR}/indexer_client.py" ]; then
    cd "${AUTO_DIR}"
    "$VENV_PY" indexer_client.py --reindex-from-history || echo "  - reindex zakonczony (sprawdz logi)"
  else
  ...
  while true; do
    ...
    if [ "$USE_SYSTEMD" -eq 1 ]; then
      ...
      if [ "$STATUS" = "active" ]; then
        echo -e "  Daemon: ${GREEN}AKTYWNY${RESET} | ${SERVICE_NAME} (systemd)"
      else
        echo -e "  Daemon: ${RED}NIEAKTYWNY${RESET} | ${SERVICE_NAME} (systemd)"
      fi
    else
      if [ -f "${PIDFILE}" ] && kill -0 "$(cat "${PIDFILE}" 2>/dev/null)" 2>/dev/null; then
        echo -e "  Daemon: ${GREEN}AKTYWNY${RESET} | ${SERVICE_NAME} (SysVinit)"
      else
        echo -e "  Daemon: ${RED}NIEAKTYWNY${RESET} | ${SERVICE_NAME} (SysVinit)"
      fi
    fi
    echo
