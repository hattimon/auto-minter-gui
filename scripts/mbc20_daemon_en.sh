#!/bin/bash
# mbc20_daemon.sh - Universal headless manager for the MBC20 / Moltbook daemon
...
if isinstance(d, dict) and 'profiles' in d:
    with open(p, 'w') as f:
        json.dump(d['profiles'], f, indent=2)
...
set_kv() {
  local key="$1" val="$2"
  if [ -z "$val" ]; then
    echo " - ${key}: kept existing value (empty input)"
    return 0
  fi
  mkdir -p "${AUTO_DIR}"
  touch "$ENV_FILE"
...
patch_daemon_script() {
  ...
  if [ ! -f "$script" ]; then
    echo " - skipping daemon patch: ${script} not found"
    return 0
  fi
...
def get_post_description(profile: dict) -> str:
    # uses extra_comment from profile created by the shell script
...
if n_cut:
    ...
    print(" (Warning: main() not found; adding a new one).")
...
install_headless() {
  ...
  if [ -z "$OLD_MOLT" ]; then
    ...
    read -p "MOLTBOOK_API_KEY (empty = keep): " NEW_MOLT
    set_kv "MOLTBOOK_API_KEY" "$NEW_MOLT"
  fi
...
JSONEOF
  ...
  # start_daemon.sh – PID = python child (works with both systemd and init.d) + PIDFILE as APP_USER
...
# Default-Stop:      0 1 6
# Short-Description: Moltbook MBC20 Auto-Minter (${APP_BASENAME})
### END INIT INFO

DAEMON_DIR="${AUTO_DIR}"
DAEMON_SH="${APP_DIR}/start_daemon.sh"
PIDFILE="${PIDFILE}"
LOGFILE="${LOG_FILE}"
...
case "\$1" in
  start)
    ...
    start-stop-daemon --start --background --pidfile "\$PIDFILE" \
                      --chdir "\$DAEMON_DIR" --exec "\$DAEMON_SH"
    ;;
  stop)
...
add_profile() {
  ...
  read -p "Additional comment (empty=none): " COMMENT

  RAND_ID=$(tr -dc A-Za-z0-9 </dev/urandom | head -c 10)

  "$VENV_PY" - "$PROFILES_JSON" "$PROFILE_NAME" "$TOKEN_TICK" "$SUBMOLT" \
             "$AMOUNT" "$TITLE" "$COMMENT" "$RAND_ID" << 'PYEOF'
import json, sys

p, name, tick, sub, amt, title, comment, rid = sys.argv[1:]
with open(p) as f:
    d = json.load(f)
if not isinstance(d, list):
    d = d.get('profiles', []) if isinstance(d, dict) else []
...
profile = {
    ...
    "ticker": tick,
    "submolt": sub,
    "amt": int(amt) if amt.isdigit() else amt,
    "amount_per_mint": int(amt) if amt.isdigit() else amt,
    "title": title,
...
reindex_history() {
  ...
  if [ -f "${AUTO_DIR}/indexer_client.py" ]; then
    cd "${AUTO_DIR}"
    "$VENV_PY" indexer_client.py --reindex-from-history || \
      echo " - reindex finished (check logs)"
  else
...
main_menu() {
  while true; do
    ...
    case "$choice" in
      1) install_headless ;;
      2) add_profile ;;
      3) edit_profile ;;     # TODO: translate / implement body
      4) delete_profile ;;   # TODO: translate / implement body
      5) select_profile ;;   # TODO: translate / implement body
      6) tail_logs ;;
      7) edit_env ;;
      8) toggle_autostart ;;
      9) reindex_history ;;
