...
class AutoMintWorker(QThread):
    ...
    def run(self):
        from moltbook_auto_minter import AutoMinter, moltbook_client

        def logfn(msg: str):
            self.log_signal.emit(f"[AUTO-MINT] {msg}")

        def choose_api_key():
            key = self.gui.get_next_moltbook_key()
            if key:
                moltbook_client.set_api_key(key)
                return key
            return None
        ...
        try:
            minter = AutoMinter(
                ...
                choose_api_key_fn=choose_api_key,  # nowy parametr
            )
            minter.run_loop()
        finally:
...
class Mbc20InscriptionGUI(QWidget):
    def __init__(self):
        ...
        cfg = load_moltbook_config_from_file()
        self.multi_key_enabled: bool = cfg.get("multi_key_enabled", False)
        self.moltbook_slots: list[dict] = cfg.get("slots", [])
        if not self.moltbook_slots:
            # fallback, jeśli coś poszło nie tak
            self.moltbook_slots = [
                {"name": "Main", "key": os.getenv("MOLTBOOK_API_KEY", ""), "active": True}
            ]

    def get_active_slots(self) -> list[dict]:
        return [s for s in self.moltbook_slots if s.get("active") and s.get("key")]

    def get_single_active_key(self) -> str | None:
    ...
    def get_next_moltbook_key(self) -> str | None:
        if not self.multi_key_enabled:
            return self.get_single_active_key()
        slots = self.get_active_slots()
        if not slots:
            return None
        if not hasattr(self, "_multi_key_index"):
            self._multi_key_index = 0
        key = slots[self._multi_key_index]["key"]
        self._multi_key_index = (self._multi_key_index + 1) % len(slots)
        return key
    ...
    KEYS_FILE = Path("moltbook_keys.json")


    def load_moltbook_config_from_file():
        if not KEYS_FILE.exists():
            # domyślna konfiguracja na podstawie aktualnego .env
            from dotenv import dotenv_values
            env = dotenv_values(".env")
            key = env.get("MOLTBOOK_API_KEY", "")
            return {
                "multi_key_enabled": False,
                "slots": [
                    {"name": "Main", "key": key, "active": True}
                ]
            }
        try:
    ...
    def save_moltbook_config_to_file(cfg: dict):
        KEYS_FILE.write_text(json.dumps(cfg, indent=2), encoding="utf-8")
    ...
    def save_env_from_widget(self):
        try:
            ...
            QMessageBox.critical(self, self.tr["error"], str(e))
                        ...
                        # zbierz sloty z UI
            slots = []
            for row in self.env_moltbook_slots_widgets:
                name = row["name_edit"].text()
                key = row["key_edit"].text()
                active = row["active_checkbox"].isChecked()
                slots.append({"name": name, "key": key, "active": active})

            self.moltbook_slots = slots
            self.multi_key_enabled = self.env_multi_key_checkbox.isChecked()

            active_slots = [s for s in slots if s["active"] and s["key"]]

            if not self.multi_key_enabled:
                # single-key: wymuś dokładnie jeden aktywny
                if not active_slots and slots:
                    # jeśli żaden nieaktywowany, ustaw pierwszy z niepustym key jako active
                    for s in slots:
                        if s["key"]:
                            s["active"] = True
                            active_slots = [s]
                            break
                active_key = active_slots[0]["key"] if active_slots else ""
                new_env["MOLTBOOK_API_KEY"] = active_key
            else:
                # multi-key: weź pierwszy aktywny jako fallback
                fallback_key = active_slots[0]["key"] if active_slots else ""
                new_env["MOLTBOOK_API_KEY"] = fallback_key

            save_moltbook_config_to_file({
                "multi_key_enabled": self.multi_key_enabled,
                "slots": self.moltbook_slots,
            })
    ...
    def load_env_to_widget(self):
        ...
        # wczytaj config multi-key z pliku i odśwież UI slotów
        cfg = load_moltbook_config_from_file()
        self.multi_key_enabled = cfg.get("multi_key_enabled", False)
        self.moltbook_slots = cfg.get("slots", self.moltbook_slots)

        # ustaw checkbox "Unlock multi key support"
        self.env_multi_key_checkbox.setChecked(self.multi_key_enabled)

        # TODO: tutaj wypełnij listę slotów w UI na podstawie self.moltbook_slots
        # np. dla każdego slotu tworzysz wiersz:
        #  - QLineEdit na name,
        #  - QLineEdit na key,
        #  - QCheckBox na active
        # i zapisujesz powiązanie w self.env_moltbook_slots_widgets
    ...
    def create_inscription_post(self):
        ...
        if not self.multi_key_enabled:
            key = self.get_single_active_key()
            moltbook_client.set_api_key(key)
            self.log(f"[MANUAL] Using single key")
            self._do_single_mint()
        else:
            slots = self.get_active_slots()
            if not slots:
                QMessageBox.warning(self, "Error", "No active Moltbook API keys.")
                return
            for i, slot in enumerate(slots):
                moltbook_client.set_api_key(slot["key"])
                self.log(f"[MANUAL MULTI] Using key {slot.get('name','') or i}")
                self._do_single_mint()
