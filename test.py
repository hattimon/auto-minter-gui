...
LANG_STRINGS = {
    ...
    "pl": {
        ...
        "env_multi_key_label": "Odblokuj obsługę wielu kluczy (Experymentalne!)",
...
class Mbc20InscriptionGUI(QWidget):
    ...
    def init_ui(self):
        ...
        self.env_openai_key_edit.setPlaceholderText("OPENAI_API_KEY=sk-proj-yourKey")
        ...
        self.env_openai_model_edit.setPlaceholderText("OPENAI_MODEL=gpt-4.1-mini")
    ...
    def parse_env_api_slots(self, text: str):
        ...
        - slots_moltbook: lista slotów MOLTBOOK_API_KEY
          z labelami wziętymi z komentarzy '#N nazwa' NAD kluczem,
        ...
        for idx, line in enumerate(lines):
            ...
            if stripped.startswith("#MOLTBOOK_API_KEY") or stripped.startswith("MOLTBOOK_API_KEY"):
                ...
                if not label:
                    ...
                    if value.startswith("moltbook_sk_"):
                        label = value[-8:]
                    else:
                        label = value[:8]
    ...
    def rebuild_moltbook_slots_ui(self, slots_moltbook):
        ...
        for slot in slots_moltbook:
            ...
            # jeśli nie ma, użyj fallbacku (końcówka klucza)
            if not label_from_gui:
                if value.startswith("moltbook_sk_"):
                    label_from_gui = value[-8:]
                else:
                    label_from_gui = value[:8]
            ...
            label_edit = QLineEdit(label_from_gui)
            label_edit.setPlaceholderText("Opis API (np. YerbaMater)")
    ...
    def save_env_from_widget(self):
        try:
            ...
            # 5) Finalny tekst: istniejące linie + nowy blok na końcu
            final_lines = list(new_other_lines)
            if molt_lines:
